...
public class HomeViewController {
    ...
    private void loadMockData() {
        ...
        if (bannerContainer != null) {
            ...
            bannerContainer.getChildren().add(createAuctionCard("B2", "Tung Tung Tung Sahur", "25,000,000,000", "12:45:00", true));
        }
    ...
    private Node createAuctionCard(String id, String title, String price, String time, boolean isBanner) {
        try {
            ...
            // 4. (Tùy chọn) Chỉnh chiều rộng nếu nó là thẻ Banner to
            if (card instanceof Region) {
                ((Region) card).setPrefWidth(isBanner ? 580 : 280);
            }

            return card; // Trả về cái thẻ đã hoàn chỉnh

        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Lỗi load thẻ sản phẩm!");
            return new javafx.scene.control.Label("Lỗi tải thẻ"); // Trả về 1 dòng chữ báo lỗi nếu hỏng
        }
    }
